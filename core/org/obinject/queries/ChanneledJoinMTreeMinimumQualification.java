 ...
 * @param <M>
 */
public abstract class ChanneledJoinMTreeMinimumQualification<M extends Metric<M> & Entity<? super M>> extends AbstractStrategy<M> {
    ...
    public ChanneledJoinMTreeMinimumQualification(MTree<M> mtree, List<M> routeOfPoint, double range) {
        super(mtree);
                ...
                for (int j = 0; j < total; j++) {
                    ...
                    if (distParentToQuery - index.readDistanceToParent(j) <= cov + range) {
                        ...
                        for (int i = 0; i < routeOfPoint.size() - 1; i++) {
                            ...
                            if ((Math.pow(c, 2) >= Math.pow(b, 2) + Math.pow(a, 2))) {
                                ...
                                if ((b <= cov + range) && (b < minDistParentToQuery)) {
                                    // A minima distancia recebe b
                                    minDistParentToQuery = b;
                                    minDistRepresentative = b;
                                    // O id deste objeto é guardado
                                ...
                                if ((taltura <= cov + range) && (taltura < minDistParentToQuery)) {
                                    // A minima distancia recebe taltura
                        ...
                        if (minDistParentToQuery != Double.MAX_VALUE) {
                            qualifies.add(new Qualify(minSubPageId, minDistRepresentative));
                        }
                    }
                }
            } else {
