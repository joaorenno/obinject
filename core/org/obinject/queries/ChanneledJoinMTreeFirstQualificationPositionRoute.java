 ...
 * @param <M>
 */
public abstract class ChanneledJoinMTreeFirstQualificationPositionRoute<M extends Metric<M> & Entity<? super M>> extends AbstractStrategy<M> {
    ...
    public ChanneledJoinMTreeFirstQualificationPositionRoute(MTree<M> mtree, List<M> routeOfPoint, double range) {
        super(mtree);
    ...
    public Collection<M> solve() {
        ...
        // Variavel que guardara posição da rota
        int positionRoute;
        ...
        do {
            ...
            //
            positionRoute = objQualify.positionRoute;
            ...
            if (MTreeIndex.matchNodeType(node)) {
                ...
                for (int j = 0; j < total; j++) {
                    ...
                    // Desigualdade Triangular não pode ser usada
                    ...
                    for (int i = positionRoute; i < routeOfPoint.size() - 1; i++) {
                        ...
                        if ((Math.pow(c, 2) >= Math.pow(b, 2) + Math.pow(a, 2))) {
                            ...
                            if ((b <= cov + range)) {
                                qualifies.add(new Qualify(index.readSubPageId(j), b, i));
                                break;
                            }
                            ...
                            if (c <= cov + range) {
                                qualifies.add(new Qualify(index.readSubPageId(j), c, i));
                                break;
                            }
                        } else {
                            // REGIAO 2
                            ...
                            if (taltura <= cov + range) {
                                qualifies.add(new Qualify(index.readSubPageId(j), taltura, i));
                                break;
                            }
                        }
                    }
