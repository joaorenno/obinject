 ...
 */
public class RectangleKeyGenerator extends CodeGenerator<RectangleKeyDefinition> {
     ...
     * @param definition
     */
    public RectangleKeyGenerator(RectangleKeyDefinition definition) {
        super(definition);
        Object newObj;
        Object returnMethod;
        char typeReturn;
    ...
    private void generateImport() {
        ...
        code.append("import org.obinject.meta.Rectangle; ");
    ...
    private void generateBeginClass() {
        ...
        code.append("public class ").append(this.getDefinition().getClassName()).append(" extends ").
                ...
                append(" implements Rectangle<").append(this.getDefinition().getClassName()).append("> { ");
    }
    ...
    private void generateExtensions() {
        if (this.getDefinition().getExtensionsFields().isEmpty()) {
            code.append("private double extensions[] = new double[").
                    append(this.dimensions).append("];");
        }
    }
...
//    }
    ...
    private void generateExtension() {
        ...
        Object returnMethod;
        char typeReturn;
        StringBuilder codeSetExtension = new StringBuilder();
        //is not exists annotation extension
        if (this.getDefinition().getExtensionsFields().isEmpty()) {
            code.append("@Override ");
            code.append("public double getExtension(int axis){ ");
            code.append("return extensions[axis]; } ");
            code.append("@Override ");
            code.append("public void setExtension(int axis, double value){ ");
            code.append("extensions[axis]=value; } ");
        } else {
            code.append("@Override ");
            code.append("public double getExtension(int axis){ ");
            code.append("switch(axis){ ");
            codeSetExtension.append("@Override ");
            codeSetExtension.append("public void setExtension(int axis, double value){ ");
            codeSetExtension.append("switch(axis){ ");
            for (ClassField extensionField : this.getDefinition().getExtensionsFields()) {
                if (extensionField.getField().getType().isArray()) {
                    //field is array
                    try {
                        newObj = Class.forName(this.getDefinition().getPackClassName() + "."
                                + this.getDefinition().getUserClassName()).newInstance();
                        returnMethod = newObj.getClass().getDeclaredMethod(extensionField.getGet()).invoke(newObj);
                        typeReturn = returnMethod.getClass().getName().charAt(1);
                        switch (typeReturn) {
                            case 'Z':
                                sizeVector = ((boolean[]) returnMethod).length;
                        ...
                        for (int i = 0; i < sizeVector; i++) {
                            //get generation
                            code.append("case ").append(dim + i).append(": return ").
                                    append(extensionField.getGet()).append("()[").
                                    append(i).append("]; ");
                            //set generation
                            switch (typeReturn) {
                                case 'Z':
                                    codeSetExtension.append("case ").append(dim + i).append(": ").
                                            append(extensionField.getGet()).append("()[").append(i).append("] = (boolean)value; break; ");
                                    break;
                                case 'B':
                                    codeSetExtension.append("case ").append(dim + i).append(": ").
                                            append(extensionField.getGet()).append("()[").append(i).append("] = (byte)value; break; ");
                                    break;
                                case 'C':
                                    codeSetExtension.append("case ").append(dim + i).append(": ").
                                            append(extensionField.getGet()).append("()[").append(i).append("] = (char)value; break; ");
                                    break;
                                case 'D':
                                    codeSetExtension.append("case ").append(dim + i).append(": ").
                                            append(extensionField.getGet()).append("()[").append(i).append("] = value; break; ");
                                    break;
                                case 'F':
                                    codeSetExtension.append("case ").append(dim + i).append(": ").
                                            append(extensionField.getGet()).append("()[").append(i).append("] = (float)value; break; ");
                                    break;
                                case 'I':
                                    codeSetExtension.append("case ").append(dim + i).append(": ").
                                            append(extensionField.getGet()).append("()[").append(i).append("] = (int)value; break; ");
                                    break;
                                case 'J':
                                    codeSetExtension.append("case ").append(dim + i).append(": ").
                                            append(extensionField.getGet()).append("()[").append(i).append("] = (long)value; break; ");
                                    break;
                                case 'S':
                                    codeSetExtension.append("case ").append(dim + i).append(": ").
                                            append(extensionField.getGet()).append("()[").append(i).append("] = (short)value; break; ");
                                    break;
                            }
                        }
                        //increment dim
                    ...
                    // field is not array
                    //get generation
                    code.append("case ").append(dim).append(": return ").append(extensionField.getGet()).
                            append("(); ");
                    //set generation
                    if (extensionField.getField().getType() == double.class) {
                        //field is double
                        codeSetExtension.append("case ").append(dim).append(": ").append(extensionField.getSet()).
                                append("(value); break; ");
                    } else if (extensionField.getField().getType() == float.class) {
                        //field is float
                        codeSetExtension.append("case ").append(dim).append(": ").append(extensionField.getSet()).
                                append("((float)value); break; ");
                    } else if (extensionField.getField().getType() == byte.class) {
                        //field is byte
                        codeSetExtension.append("case ").append(dim).append(": ").append(extensionField.getSet()).
                                append("((byte)value); break; ");
                    } else if (extensionField.getField().getType() == short.class) {
                        //field is short
                        codeSetExtension.append("case ").append(dim).append(": ").append(extensionField.getSet()).
                                append("((short)value); break; ");
                    } else if (extensionField.getField().getType() == int.class) {
                        //field is int
                        codeSetExtension.append("case ").append(dim).append(": ").append(extensionField.getSet()).
                                append("((int)value); break; ");
                    } else if (extensionField.getField().getType() == long.class) {
                        //field is long
                        codeSetExtension.append("case ").append(dim).append(": ").append(extensionField.getSet()).
                                append("((long)value); break; ");
                    } else if (extensionField.getField().getType() == char.class) {
                        //field is char
                        codeSetExtension.append("case ").append(dim).append(": ").append(extensionField.getSet()).
                                append("((char)value); break; ");
                    }
                    dim++;
                }
            }
            code.append("default: return 0; ");
            code.append("} } ");
            codeSetExtension.append("} } ");
            //adding set in code
            code.append(codeSetExtension);
        }
    }
    ...
    private void generatePullKey() {
        ...
        code.append("this.setExtension(i, pull.pullDouble()); ");
    ...
    private void generatePushKey() {
        ...
        code.append("push.pushDouble(this.getExtension(i)); ");
        code.append("} } ");
    }
    ...
    private void generateSizeOfKey() {
        ...
        code.append("return Page.sizeOfDouble * 2 * this.numberOfDimensions();} ");
    }
